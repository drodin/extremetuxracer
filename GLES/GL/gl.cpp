...
GLuint qglDrawElements_currentPos = 0;

GLuint qglDrawElements_currentWindowSize = 0;
...
void qglDrawElements (GLenum mode, GLsizei count, GLenum type, const GLvoid *indices)
{
    ...
    const unsigned int maxBufferSize = USHRT_MAX;
    ...
    if(type == GL_UNSIGNED_INT) {
        ...
        unsigned int newWindowSize = qglDrawElements_currentWindowSize;
        GLuint min = UINT_MAX, max = 0;
        GLuint newPos =     qglDrawElements_currentPos;
        for(i = 0; i<count;i++) {
        ...
        unsigned int idealValue = (unsigned int)max - (unsigned int)min + 1;

        const unsigned int minBufferSize = 0;// USHRT_MAX/10;
...
//            assert((int)val - (int)currentPos >= 0);
            unsigned int newVal = (unsigned int)val - (unsigned int)newPos;
...
map <GLint, int > TextureWidth, TextureHeight;
...
void qglTexImage2D (GLenum target, GLint level, GLint internalformat, GLsizei width, GLsizei height, GLint border, GLenum format, GLenum type, const GLvoid *pixels) {
	...
	TextureWidth[textureId] = nextp2(width);
	TextureHeight[textureId] = nextp2(height);

    glTexImage2D(target, level, format, TextureWidth[textureId], TextureHeight[textureId], border, format, type, NULL);
...
GLint qgluBuild2DMipmaps(GLenum target, GLint internalFormat, GLsizei width, GLsizei height, GLenum format, GLenum type, const void * data) {
    ...
    qglTexImage2D(target, 0, format, width, height, 0, format, type, data);
    return 0;
}
...
void qglGetTexLevelParameteriv( GLenum target, GLint level, GLenum pname, GLint *params ) {
	...
	if (textureId) {
		...
		case GL_TEXTURE_WIDTH:
			*params = TextureWidth[textureId];
			return;
		case GL_TEXTURE_HEIGHT:
			*params = TextureHeight[textureId];
			return;
		}
	}
...
void qglDeleteTextures(GLsizei n, const GLuint *textures) {
	int i;
	for (i=0;i<n;i++) {
		TextureWidth.erase(textures[i]);
		TextureHeight.erase(textures[i]);
	}
